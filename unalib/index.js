...
module.exports = {
  ...
  sanitizeMessage: function(message) {
    ...
    return message.replace(/<script[^>]*>([\S\s]*?)<\/script>/gmi, '')
                  .replace(/<\/?\w(?:[^"'>]|"[^"]*"|'[^']*')*>/gmi, '');
  },
  ...
  validateMessage: function(msg) {
    ...
    // Validar si el mensaje es una URL válida de imagen o video
      ...
      if (this.is_image_url(obj.mensaje)) {
        ...
        obj.mensaje = "<img src='" + obj.mensaje + "' style='max-width: 200px; max-height: 200px;' />";
      } else if (this.is_video_url(obj.mensaje)) {
        console.log("Es un video!");
        obj.mensaje = "<video controls style='max-width: 320px; max-height: 240px;'><source src='" + obj.mensaje + "' type='video/mp4'></video>";
      } else {
        // Si es una URL válida pero no es imagen ni video, simplemente muestra el link
        obj.mensaje = "<a href='" + obj.mensaje + "' target='_blank'>" + obj.mensaje + "</a>";
      }
    } else {
